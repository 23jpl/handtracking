...
def draw_box_on_image(num_hands_detect, score_thresh, scores, boxes, im_width, im_height, image_np, ASLmodel):
    for i in range(num_hands_detect):
        if (scores[i] > score_thresh):
            (left, right, top, bottom) = (boxes[i][1] * im_width-50, boxes[i][3] * im_width+50,
                                          boxes[i][0] * im_height-100, boxes[i][2] * im_height)
            p1 = (int(left), int(top))
            ...
            message = str(round(scores[i],2))+", "+sign
            font = cv2.FONT_HERSHEY_SIMPLEX
            topmiddle = (int((int(left)+int(right))/2-60), int(top)-10)
            cv2.putText(image_np, message, topmiddle, font, 1, (77, 255, 9), 2, cv2.LINE_4)
...
def detect_sign(hand, ASLmodel):
    try:
        ...
        grey = (0.2989*resized[:,:,0]+0.5870*resized[:,:,1]+0.1140*resized[:,:,2]) / 255
       # plt.imshow(resized, vmin=0, vmax=1)
        plt.imshow(grey, cmap='gray', vmin=0, vmax=1)
